...
public class MessageFormatterTest {
	...
	public void noScenarioEmpty() {
		assertThat(new MessageFormatter("", null, null).scenarioPrefix).isNotNull().isEmpty();
	}
	...
	public void noScenarioFailNoDescriptionNoArgs() {
		DefaultStepVerifierBuilder.Event event = new DefaultStepVerifierBuilder.NoEvent(
			Duration.ofMillis(5),
			""
		);
	...
	public void noScenarioAssertionError() {
		assertThat(noScenario.assertionError("plain")).hasMessage("plain").hasNoCause();
	}
	...
	public void noScenarioAssertionErrorWithNullCause() {
		assertThat(noScenario.assertionError("plain", null)).hasMessage("plain").hasNoCause();
	}
	...
	static final MessageFormatter withScenario = new MessageFormatter(
	...
	public void withScenarioFailDescriptionNoArgs() {
		...
		assertThat(withScenario.fail(event, "details"))
			.hasMessage(
				"[MessageFormatterTest] expectation \"eventDescription\" failed (details)"
			);
	}
	...
	public void withScenarioFailDescriptionHasArgs() {
		...
		assertThat(withScenario.fail(event, "details = %s", "bar"))
			.hasMessage(
				"[MessageFormatterTest] expectation \"eventDescription\" failed (details = bar)"
			);
	}
	...
	public void withScenarioFailOptional() {
		assertThat(withScenario.failOptional(null, "foo"))
			.hasValueSatisfying(
				ae -> assertThat(ae).hasMessage("[MessageFormatterTest] expectation failed (foo)")
			);
	}
	...
	static final MessageFormatter withCustomFormatter = new MessageFormatter(
		"withCustomFormatter",
		ValueFormatters.forClass(String.class, o -> o.getClass().getSimpleName() + "=>" + o),
		Arrays.asList(
			ValueFormatters.DEFAULT_SIGNAL_EXTRACTOR,
			ValueFormatters.DEFAULT_ITERABLE_EXTRACTOR,
	...
	public void withCustomFormatterFailDescriptionNoArgs() {
		...
		assertThat(withCustomFormatter.fail(event, "details"))
			.hasMessage(
				"[withCustomFormatter] expectation \"eventDescription\" failed (details)"
			);
	}
	...
	public void withCustomFormatterFailDescriptionHasArgs() {
		...
		assertThat(withCustomFormatter.fail(event, "details = %s", "bar"))
			.hasMessage(
				"[withCustomFormatter] expectation \"eventDescription\" failed (details = String=>bar)"
			);
	}
	...
	public void withCustomFormatterFailOptional() {
		assertThat(withCustomFormatter.failOptional(null, "foo"))
			.hasValueSatisfying(
				ae -> assertThat(ae).hasMessage("[withCustomFormatter] expectation failed (foo)")
			);
	}
	...
	public void withCustomFormatterFormatSignal() {
		assertThat(
			withCustomFormatter.format(
				"expectation %s expected %s got %s",
				Signal.next("foo"),
				"foo",
				Signal.next("bar")
			)
			...
			.isEqualTo(
				"expectation onNext(String=>foo) expected String=>foo got onNext(String=>bar)"
			);
	}
	...
	public void withCustomFormatterFormatIterable() {
		assertThat(
			...
			.isEqualTo(
				"expectation [String=>foo, String=>bar] expected String=>foo got [String=>bar]"
			);
	}
