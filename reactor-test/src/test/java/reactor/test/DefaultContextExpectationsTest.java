...
public class DefaultContextExpectationsTest {
	...
	private void assertContextExpectation(
		...
		final DefaultContextExpectations<Integer> base = new DefaultContextExpectations<>(
			step,
			new MessageFormatter(null, null, null)
		);

		expectations.apply(base).then().expectNextCount(count).verifyComplete();
	}
	...
	public void contextAccessibleLastInChain() {
		assertContextExpectation(
			s -> s.take(3).contextWrite(Context.of("a", "b")),
			e -> e,
			3
		);
	}
	...
	public void contextAccessibleFirstInChain() {
		assertContextExpectation(
			s -> s.contextWrite(Context.of("a", "b")).take(3),
			e -> e,
			3
		);
	}
	...
	public void notContextAccessibleDueToPublisher() {
		...
		DefaultContextExpectations<Integer> expectations = new DefaultContextExpectations<>(
			step,
			new MessageFormatter(null, null, null)
		);
	...
	public void hasKey() throws Exception {
		assertContextExpectation(
			s -> s.contextWrite(Context.of("foo", "bar")),
			e -> e.hasKey("foo")
		);
	}
	...
	public void notHasKey() throws Exception {
		assertContextExpectationFails(
			...
			.withMessageStartingWith("" + "Key bar not found\n" + "Context: Context1{foo=bar}");
	}

	@Test
	public void hasSize() throws Exception {
		assertContextExpectation(
			s -> s.contextWrite(Context.of("foo", "bar", "foobar", "baz")),
			e -> e.hasSize(2)
		);
	}
	...
	public void notHasSize() throws Exception {
		assertContextExpectationFails(
				s ->
					s
						.contextWrite(Context.of("foo", "bar", "foobar", "baz"))
	...
	public void notMatchesWithDescriptionAndScenarioName() {
		Flux<Integer> source = Flux.range(1, 10).contextWrite(Context.of("foo", "bar"));

		Step<Integer> step = StepVerifier.create(source);
		final DefaultContextExpectations<Integer> base = new DefaultContextExpectations<>(
			step,
			new MessageFormatter("scenario", null, null)
		);

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(
				base
					.matches(Objects::isNull, "someDescription")
					.then()
	...
	public void capturedOperator() {
		assertContextExpectationFails(s -> s.doOnEach(__ -> {}), e -> e.hasKey("foo"))
	...
	public void capturedOperatorWithDebug() {
		...
		assertContextExpectationFails(s -> s.doOnEach(__ -> {}), e -> e.hasKey("foo"))
			.withMessageContaining(
				"Captured at: Flux.range ⇢ at reactor.test.DefaultContextExpectationsTest.assertContextExpectation(DefaultContextExpectationsTest.java:"
			);
	}
